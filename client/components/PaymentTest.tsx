    ...
    <Card className="max-w-md mx-auto m-4">
      ...
      <CardContent className="space-y-4">
        <div>
          ...
          <p className="text-sm">Loading: {documentsState.isLoading ? 'Yes' : 'No'}</p>
          <p className="text-sm">Error: {documentsState.error || 'None'}</p>
        </div>
        
        <div>
          <h4 className="font-semibold">Pricing State:</h4>
          <p className="text-sm">Total: ₹{pricingState.calculation.totalAmount.toFixed(2)}</p>
          <p className="text-sm">Calculating: {pricingState.isCalculating ? 'Yes' : 'No'}</p>
          <p className="text-sm">Error: {pricingState.error || 'None'}</p>
        </div>
        
        <div>
          <h4 className="font-semibold">Payment State:</h4>
          <p className="text-sm">Processing: {paymentState.isProcessing ? 'Yes' : 'No'}</p>
          <p className="text-sm">Initialized: {paymentState.currentPayment ? 'Yes' : 'No'}</p>
